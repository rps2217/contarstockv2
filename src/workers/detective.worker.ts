...
const calculateOrderMatch = (
  physicalItems: ConsolidatedItem[],
  order: ExpectedOrder
): MatchResult => {
  const details: Array<{
    barcode: string;
  ...
  unmatchedPhysical.forEach(p =>
    details.push({
      barcode: p.barcode,
      name: p.productName,
      physicalQty: p.totalQuantity,
      expectedQty: 0,
      difference: p.totalQuantity,
    })
  );
  unmatchedExpected.forEach(e =>
    details.push({
      barcode: e.barcode,
      name: e.name,
      physicalQty: 0,
...
const findSemanticNeighbors = (targetEmbedding: number[], catalog: Product[], limit = 3) => {
  return catalog
    ...
    .filter(p => p.score > 0.7) // Sensibilidad del radar

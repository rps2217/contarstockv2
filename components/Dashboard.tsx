    ...
    <div className="h-full w-full bg-slate-950 overflow-y-auto no-scrollbar pb-32">
      ...
      <header className="px-6 py-8 border-b-4 border-white/5 bg-slate-900/40 flex justify-between items-center sticky top-0 z-50 backdrop-blur-md">
          <div className="flex flex-col min-w-0">
              <span className="text-[7px] font-black text-blue-500 tracking-[0.4em] uppercase mb-1">DATA_TERMINAL_V4.2</span>
              <h1 className="text-2xl font-black text-white italic tracking-tighter uppercase leading-none truncate">MAIN_INTERFACE</h1>
          </div>
          <div className="flex flex-col items-end shrink-0">
              ...
              <span className="text-[7px] font-black text-white/30 uppercase tracking-widest">{isSyncNeeded ? 'QUEUED' : 'ACTIVE'}</span>
          </div>
      </header>

      <div className="p-4 max-w-4xl mx-auto space-y-4">
        
        <div className="bg-slate-900/80 border-4 border-white/5 p-6 flex items-center justify-between relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-8 opacity-5 rotate-12 transition-transform group-hover:scale-110">
                <Box className="w-32 h-32" />
            </div>
            <div className="relative z-10">
                <span className="text-[7px] font-black text-blue-400 uppercase tracking-[0.4em] block mb-2">PRODUCTION_COUNTER</span>
                <div className="text-7xl font-black text-white tabular-nums tracking-tighter leading-none">
                    {stats?.scansToday || 0}
                </div>
            </div>
            <div className="text-right relative z-10">
                <div className="text-[8px] font-black text-white/40 uppercase mb-1">SYSTEM_STATUS</div>
        ...
        <div className="grid grid-cols-2 gap-4">
            <button onClick={() => navigate('/database')} className="h-28 bg-slate-900/50 border-4 border-white/5 flex flex-col items-center justify-center gap-3 active:bg-blue-600 transition-all group">
                <Database className="w-6 h-6 text-blue-400 group-active:text-white" />
                <span className="text-[8px] font-black uppercase tracking-[0.3em]">Catálogo_Master</span>
            </button>
            <button onClick={() => navigate('/sync')} className={`h-28 border-4 flex flex-col items-center justify-center gap-3 transition-all ${isSyncNeeded ? 'bg-amber-600 border-amber-800' : 'bg-slate-900/50 border-white/5 text-white/30'}`}>
        ...
        <button onClick={() => navigate('/settings')} className="w-full h-14 bg-black/40 border-2 border-white/5 text-white/20 flex items-center justify-center gap-3 active:text-white active:bg-white/5 transition-all">
            <Settings className="w-4 h-4" />
            <span className="text-[8px] font-black uppercase tracking-[0.5em]">OPERATIONAL_SETUP_V4.2</span>
        </button>
      </div>
    </div>
  );

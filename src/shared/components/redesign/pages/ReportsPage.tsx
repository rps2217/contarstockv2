...
import {
  ...
  type TimePeriod,
  type ReportType,
} from './ReportsPage/reportComponents';
...
export const RedesignReportsPage: React.FC = () => {
  ...
  const countingStats = useLiveQuery(async () => {
    ...
    const prevSessions = await SessionRepository.getByDateRange(
    ...
    const trend =
      prevTotalUnits > 0
        ? Math.round(((totalUnits - prevTotalUnits) / prevTotalUnits) * 100)
        : totalUnits > 0
          ? 100
          : 0;
    return {
      ...
      totalUnits,
      avgPerSession:
        completedSessions.length > 0 ? Math.round(totalUnits / completedSessions.length) : 0,
  ...
  const inventoryStats = useLiveQuery(async () => {
    try {
      ...
      const lowStock = await db.products
        ...
        .count()
        .catch(() => 0);
      const noStock = await db.products
  ...
  const expiryStats = useLiveQuery(async () => {
    ...
    records.forEach(r => {
      const data = r.data || {};
      const mm = (data.mm || data.month) as number,
        yyyy = (data.yyyy || data.year) as number;
      if (!mm || !yyyy) return;
      const daysLeft = Math.floor(
        (new Date(yyyy, mm - 1).getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
      );
      if (daysLeft < 0) expired++;
      else if (daysLeft <= 15) critical++;
      else if (daysLeft <= 30) upcoming++;
      else safe++;
    });
  ...
  const recentSessions = useLiveQuery(
    async () => {
      ...
      return sessions.map(s => ({
        id: s.id,
        type: s.sessionType || 'counting',
        date: s.createdAt,
        items: s.totalUnits || 0,
        status: s.status,
      }));
    },
    [],
          ...
          {activeTab === 'counting' && (
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <StatCard
                  title="Sesiones"
                  value={countingStats?.sessionCount || 0}
                  icon={ClipboardCheck}
                  color="blue"
                />
                <StatCard
                  title="Total Ítems"
                  value={formatNumber(countingStats?.totalUnits || 0)}
                  trend={countingStats?.trend}
                  isPositive={countingStats?.isPositive}
                  icon={Scan}
                  color="cyan"
                />
                <StatCard
                  title="Promedio/Sesión"
                  value={countingStats?.avgPerSession || 0}
                  icon={TrendingUp}
                  color="green"
                />
                <StatCard
                  title="Pendiente Sync"
                  value={syncStats?.pendingCount || 0}
                  icon={RefreshCw}
                  color="amber"
                />
              </div>
                ...
                <div className="h-64 flex items-end justify-between gap-2">
                  {(weeklyChart?.data || [0, 0, 0, 0, 0, 0, 0]).map((value, i) => (
                    <div key={i} className="flex-1 flex flex-col justify-end gap-1 group relative">
                      ...
                      <motion.div
                        initial={{ height: 0 }}
                        animate={{ height: `${Math.max((value / maxChartValue) * 100, 2)}%` }}
          ...
          {activeTab === 'inventory' && (
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <StatCard
                  title="Productos"
                  value={formatNumber(inventoryStats?.products || 0)}
                  icon={Package}
                  color="blue"
                />
                <StatCard
                  title="Clientes"
                  value={inventoryStats?.customers || 0}
                  icon={Users}
                  color="purple"
                />
                <StatCard
                  title="Proveedores"
                  value={inventoryStats?.providers || 0}
                  icon={Truck}
                  color="amber"
                />
                <StatCard
                  title="Sin Stock"
                  value={inventoryStats?.noStock || 0}
                  icon={XCircle}
                  color="red"
                />
              </div>
                    ...
                    <div className="flex-1 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-sm font-medium text-emerald-500">
                          {(inventoryStats?.products || 0) -
                            (inventoryStats?.noStock || 0) -
                      ...
                      <div className="h-2 bg-base rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${(((inventoryStats?.products || 0) - (inventoryStats?.noStock || 0) - (inventoryStats?.lowStock || 0)) / Math.max(inventoryStats?.products || 1, 1)) * 100}%`,
                          }}
                        />
                      </div>
                      <div className="flex items-center justify-between">
                      ...
                      <div className="h-2 bg-base rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${((inventoryStats?.lowStock || 0) / Math.max(inventoryStats?.products || 1, 1)) * 100}%`,
                          }}
                        />
                      </div>
                      <div className="flex items-center justify-between">
                      ...
                      <div className="h-2 bg-base rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${((inventoryStats?.noStock || 0) / Math.max(inventoryStats?.products || 1, 1)) * 100}%`,
                          }}
                        />
                      </div>
                    </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between p-3 bg-base rounded-xl">
                      ...
                      <span className="font-bold text-primary">
                        {formatNumber(inventoryStats?.products || 0)}
                      </span>
                    </div>
                    <div className="flex items-center justify-between p-3 bg-base rounded-xl">
                    ...
                    <div className="flex items-center justify-between p-3 bg-base rounded-xl">
                      ...
                      <span className="font-bold text-amber-500">
                        {(inventoryStats?.lowStock || 0) + (inventoryStats?.noStock || 0)}
                      </span>
                    </div>
                  </div>
                </div>
          ...
          {activeTab === 'sync' && (
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <StatCard
                  title="Operaciones"
                  value={syncStats?.totalOps || 0}
                  icon={RefreshCw}
                  color="blue"
                />
                <StatCard
                  title="Exitosas"
                  value={syncStats?.successCount || 0}
                  icon={CheckCircle2}
                  color="green"
                />
                <StatCard
                  ...
                  <div className="flex-1 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-secondary">Exitosas</span>
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-secondary">Fallidas</span>
                      </div>
                      <span className="font-bold text-rose-500">
                        {(syncStats?.totalOps || 0) - (syncStats?.successCount || 0)}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Clock className="w-4 h-4 text-amber-500" />
                        <span className="text-sm text-secondary">Pendientes</span>
                      </div>
                      <span className="font-bold text-amber-500">
          ...
          {activeTab === 'expiry' && (
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <StatCard
                  title="Críticos"
                  value={expiryStats?.critical || 0}
                  icon={AlertTriangle}
                  color="amber"
                />
                <StatCard
                  title="Próximos (30d)"
                  value={expiryStats?.upcoming || 0}
                  icon={Clock}
                  color="cyan"
                />
              </div>
                ...
                <div className="flex items-center gap-6">
                  <div className="flex-1 space-y-3">
                    {[
                      ...
                      {
                        label: 'Críticos (<15d)',
                        count: expiryStats?.critical || 0,
                        color: 'bg-amber-500',
                        textColor: 'text-amber-500',
                      },
                      {
                        label: 'Próximos (<30d)',
                        count: expiryStats?.upcoming || 0,
                        color: 'bg-cyan-500',
                        textColor: 'text-cyan-500',
                      },
                      {
                        label: 'Vigentes',
                        count: expiryStats?.safe || 0,
                      ...
                      <div key={item.label}>
                        ...
                        <div className="h-2 bg-base rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${(item.count / Math.max(expiryStats?.total || 1, 1)) * 100}%`,
                            }}
                          />
                        </div>
                      </div>
            ...
            {recentSessions.length === 0 ? (
              ...
              <div className="divide-y divide-subtle">
                {recentSessions.map(session => (
                  <div
                    key={session.id}
                    className="px-6 py-4 flex items-center justify-between hover:bg-elevated transition-colors"
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={cn(

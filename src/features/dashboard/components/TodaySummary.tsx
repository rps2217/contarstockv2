...
import { ClipboardCheck, Scan, Package, TrendingUp, CheckCircle2, Clock } from 'lucide-react';
...
export const TodaySummary: React.FC<TodaySummaryProps> = memo(
  ({ sessionsCompleted, totalScanned, totalUnits, trend = 0, isDark = true }) => {
    const stats = [
      {
      ...
      <div className="bg-surface/50 border border-subtle/60 rounded-2xl p-5">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-semibold text-slate-100">Resumen de hoy</h3>
              <p className="text-xs text-slate-500">Tu actividad del día</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {stats.map((stat, index) => (
            ...
            >
              ...
              {/* Value */}
              <div className="text-2xl font-bold text-slate-100">{stat.value.toLocaleString()}</div>
        ...
        {sessionsCompleted > 0 && (
          ...
          >
            ...
            <span className="text-xs text-muted">
              {sessionsCompleted} sesión{sessionsCompleted !== 1 ? 'es' : ''} completada
              {sessionsCompleted !== 1 ? 's' : ''} hoy
            </span>
          </motion.div>
        )}
      </div>

...
export const SliceFilters: React.FC<Props> = ({
  ...
  const accentBlue = isLight
    ? 'text-blue-600 bg-blue-50 border-blue-200'
    : isHighContrast
      ? 'text-yellow-400 bg-yellow-900/20 border-yellow-400/30'
      : 'text-blue-400 bg-blue-500/10 border-blue-500/20';
  const accentGreen = isLight
    ? 'text-emerald-600'
    : isHighContrast
      ? 'text-yellow-400'
      : 'text-emerald-500';
  const accentGreenBg = isLight
    ? 'hover:bg-emerald-50'
    : isHighContrast
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        className={`flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b ${borderMain}`}
      >
        <div className="space-y-1">
          <span
            className={`text-[9px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded-full border ${accentBlue}`}
          >
          ...
          <p className={`text-xs leading-relaxed max-w-2xl ${textSecondary}`}>
      ...
      <div
        className={`${bgSecondary} border ${borderSubtle} p-3.5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs`}
      >
        <div className="flex items-center gap-2">
          <SlidersHorizontal
            className={`w-4 h-4 ${isLight ? 'text-blue-500' : isHighContrast ? 'text-yellow-400' : 'text-blue-400'}`}
          />
          <span className={`font-bold font-mono ${textSecondary}`}>Filtro Condicional:</span>
          <code
            className={`px-2 py-0.5 rounded border font-mono font-bold ${isLight ? 'text-blue-600 bg-blue-50 border-blue-200' : isHighContrast ? 'text-yellow-300 bg-yellow-900/20 border-yellow-400/30' : 'text-blue-400 bg-blue-500/5 border-blue-500/10'}`}
          >
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5">
            ...
            <span
              className={`w-2 h-2 rounded-full ${activeSlice.allowEdits ? (isLight ? 'bg-emerald-500' : 'bg-emerald-500') : 'bg-rose-500'}`}
            />
            <span className={`text-[10px] font-bold uppercase ${textSecondary}`}>
              {activeSlice.allowEdits ? 'Sí' : 'No'}
            </span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className={`${textMuted} text-[10px] uppercase font-bold`}>Borrado:</span>
            <span
              className={`w-2 h-2 rounded-full ${activeSlice.allowDeletes ? 'bg-emerald-500' : 'bg-rose-500'}`}
            />
            <span className={`text-[10px] font-bold uppercase ${textSecondary}`}>
              {activeSlice.allowDeletes ? 'Sí' : 'No'}
            </span>
          </div>
        </div>
      </div>
      ...
      {/* Info footer */}
      <div
        className={`p-3 border rounded-2xl flex gap-2.5 ${isLight ? 'bg-blue-50 border-blue-200' : isHighContrast ? 'bg-yellow-900/20 border-yellow-400/30' : 'bg-blue-500/5 border-blue-500/10'}`}
      >
        <Info
          className={`w-4 h-4 shrink-0 mt-0.5 ${isLight ? 'text-blue-500' : isHighContrast ? 'text-yellow-400' : 'text-blue-400'}`}
        />
        <p className={`text-[10px] leading-normal ${textSecondary}`}>
          <strong>¿Qué es un Slice?</strong> Es una "segmentación de tabla" como en AppSheet. En
          lugar de procesar millones de filas, diseñas vistas compactas para la línea operativa.
        </p>
      </div>
      ...
      <div className={`flex justify-between items-center px-1 ${textMuted} text-xs`}>
        <span className="font-bold uppercase tracking-wider">
          Total en Slice: <span className={`font-mono ${textPrimary}`}>{filteredCount}</span>{' '}
          registros de <span className={`font-mono ${textSecondary}`}>{totalCount}</span>
        </span>

...
export const QueueManager: React.FC<Props> = ({ isOpen, onClose, drafts, onDelete, onDiscardAll, onFinalize }) => {
  return (
    <Modal 
      isOpen={isOpen} 
      onClose={onClose}
    ...
    >
      ...
      <div className="p-6 border-t border-white/10 bg-slate-900 absolute bottom-0 left-0 right-0 z-20 pb-safe-area flex gap-3">
        ...
        <button 
          onClick={onFinalize} 
          disabled={drafts.length === 0} 
          className="flex-[2] bg-blue-600 text-white hover:bg-blue-500 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all shadow-xl disabled:opacity-30 disabled:pointer-events-none"
        >
          Finalizar Recepción
        </button>
      </div>

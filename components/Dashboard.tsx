  ...
  return (
    <div className="w-full px-4 pt-6 max-w-lg mx-auto page-transition">
      ...
      <div className="grid grid-cols-1 gap-3 mb-6">
          <div className="bg-slate-900 dark:bg-blue-900/20 text-white p-6 rounded-[2.5rem] flex items-center justify-between shadow-xl relative overflow-hidden">
          ...
          {stats?.pendingSync && stats.pendingSync > 0 ? (
              <button 
                ...
                className="bg-orange-50 dark:bg-orange-900/20 border-2 border-orange-200 dark:border-orange-500/20 p-5 rounded-[2rem] flex items-center justify-between animate-pulse"
              >
                  <div className="flex items-center gap-4">
                      <div className="bg-orange-100 dark:bg-orange-900/40 p-3 rounded-xl"><Clock className="w-6 h-6 text-orange-600" /></div>
                      <div className="text-left">
                          <div className="text-[10px] font-black text-orange-700 dark:text-orange-400 uppercase tracking-widest">Pendiente Sincro</div>
                          <div className="text-2xl font-black text-orange-950 dark:text-orange-100 leading-none">{stats.pendingSync} <span className="text-xs">unid.</span></div>
                      </div>
                  </div>
      ...
      <div className="grid grid-cols-1 gap-3">
        {menuItems.map((item, idx) => (
          <button 
            ...
            className={`bg-white dark:bg-slate-900 border-2 ${item.border} dark:border-white/5 p-4 rounded-[2.2rem] flex items-center gap-4 text-left transition-all active:scale-[0.97] shadow-sm hover:shadow-md`}
          >
            <div className={`p-4 rounded-2xl ${item.bg} dark:bg-white/5 ${item.color} shrink-0`}>

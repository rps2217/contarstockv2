...
/**
 * MOTOR DE ESCRITURA MASIVA DE VENCIMIENTOS
 * Envía múltiples registros de vencimiento a la nube en una sola llamada.
 */
export const bulkAddExpirationsToCloud = async (items: any[]) => {
  try {
    const result = await cloudApi.post('bulk_add_expirations', { items });
    if (!result.success) {
      throw new Error(result.error || 'Error desconocido al guardar masivamente en la nube');
    }
    return result;
  } catch (e: any) {
    logger.warn("BULK_ADD_EXPIRATION_OFFLINE", `Error guardando masivamente: ${e.message}.`);
    // En caso masivo, no lo mandamos a la cola individual por ahora para evitar saturación
    // El usuario deberá reintentar la sincronización masiva
    throw e;
  }
};
...
/**
 * MOTOR DE ELIMINACIÓN MASIVA DE VENCIMIENTOS
 */
export const bulkRemoveExpirationsFromCloud = async (claves: string[]) => {
  try {
    const result = await cloudApi.post('bulk_remove_expirations', { claves });
    if (!result.success) {
      throw new Error(result.error || 'Error desconocido al eliminar masivamente en la nube');
    }
    return result;
  } catch (e: any) {
    logger.warn("BULK_REMOVE_EXPIRATION_OFFLINE", `Error eliminando masivamente: ${e.message}.`);
    throw e;
  }
};

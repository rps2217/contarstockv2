...
import { ScanLine, Container, Cloud, Settings, Zap, Activity } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const stats = useLiveQuery(async () => {
      ...
      return { scansToday, certifiedCount };
  }, []);

  const actions = [
    { title: "CONTAR", sub: "Inventario", icon: ScanLine, path: "/reports", color: "text-blue-700", bg: "bg-blue-50" },
    { title: "RECIBIR", sub: "Bultos", icon: Container, path: "/reception", color: "text-indigo-700", bg: "bg-indigo-50" },
    { title: "NUBE", sub: "Sync", icon: Cloud, path: "/sync", color: "text-emerald-800", bg: "bg-emerald-50" },
    { title: "AJUSTES", sub: "Sistema", icon: Settings, path: "/settings", color: "text-slate-800", bg: "bg-slate-50" },
  ];
      ...
      <header className="mb-6 flex justify-between items-center px-1">
          <div>
            <h1 className="text-3xl font-black tracking-tighter text-slate-900 dark:text-white uppercase leading-none italic">
                LogiCount <span className="text-blue-600">Core</span>
            </h1>
          </div>
      </header>

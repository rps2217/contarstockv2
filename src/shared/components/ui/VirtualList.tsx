 ...
 */
export interface VirtualRowProps<T, R = unknown> {
  index: number;
  item: T;
  data: R & { items: T[] };
...
interface VirtualListProps<T, R = unknown> {
  items: T[];
  itemHeight: number;
  renderRow: React.ComponentType<VirtualRowProps<T, R>>;
  rowData?: R;
  onEndReached?: () => void;
...
export const VirtualList = <T,>({
  ...
  const combinedRowData = useMemo(() => {
    return rowData !== undefined ? { ...rowData, items } : { items };
  }, [rowData, items]);
  ...
  const getKey = useCallback(
    (item: T, index: number) => {
      ...
      return (
        (item as { id?: string; barcode?: string }).id ||
  ...
  if (items.length === 0) {
    return (
      <div
        className={`h-full w-full flex flex-col items-center justify-center text-secondary ${className}`}
      >
        {emptyState || (
          <>
            <Package className="w-12 h-12 mb-3 opacity-20" />
            <p className="text-[10px] font-black uppercase tracking-widest">
              Cola de Trabajo Vacía
            </p>
          </>
        )}
      </div>
      ...
      >
        {visibleItems.map((item, localIndex) => {
          ...
          return (
            ...
            >
              <RowComponent index={globalIndex} item={item} data={combinedRowData} />
            </div>
          );
        })}
      </div>

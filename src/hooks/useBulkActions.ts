...
export interface BulkField<T = any> {
  ...
  required?: boolean;
  condition?: (data: Partial<T>) => boolean; // Mostrar solo si se cumple
}
...
export interface BulkEditConfig<T = any> {
  ...
  description: string;
  fields: BulkField<T>[];
  onApply: (selectedIds: string[], values: Record<string, any>, selectedItems: T[]) => Promise<void>;
}

export interface BulkActionsConfig<T = any> {
  /** Módulo al que pertenece (para logs) */
  module: string;
  /** Obtener el identificador único de cada item */
  getItemId: (item: T) => string;
  /** Acciones masivas disponibles */
  actions: BulkAction<T>[];
  /** Configuración del modal de edición masiva (opcional) */
  bulkEdit?: BulkEditConfig<T>;
  /** Función para editar múltiples items */
  onBulkUpdate?: (ids: string[], values: Record<string, any>, items: T[]) => Promise<void>;
  /** Función para eliminar múltiples items */
  onBulkDelete?: (ids: string[], items: T[]) => Promise<void>;
  /** Función para exportar a CSV */
  onExport?: (ids: string[], items: T[]) => Promise<void>;
}
...
import { X, Trash2, Edit3, Download, Check, Square } from 'lucide-react';
...
export function BulkActionBar<T = any>({
  ...
  const accentClass = theme === 'high-contrast' ? 'bg-yellow-500 text-black' : 'bg-blue-600 text-white';

  return (
    <div className={`fixed bottom-0 left-0 right-0 z-50 ${bgClass} border-t-4 ${theme === 'dark' ? 'border-slate-700' : theme === 'light' ? 'border-slate-200' : 'border-yellow-400'} ${className}`}>
...
// ============================================================
// COMPONENTE: BulkEditModal (Genérico)
// ============================================================
...
export function BulkEditModal<T = any>({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const field of config.fields) {
      if (field.required && !values[field.key as string]) {
    ...
    try {
      const ids = selectedItems.map(item => (item as any).id);
...
// ============================================================
// HELPER: Crear acciones estándar
// ============================================================

export function createStandardBulkActions<T>(
  config: {

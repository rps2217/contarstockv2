...
/**
 * HUD INDUSTRIAL v4.5
 * Optimizado para PDAs: Maximiza la visibilidad de la cantidad y protege contra renderizado negativo.
 */
    ...
    return (
        <div className={`h-[32dvh] relative flex flex-col overflow-hidden border-b-4 border-black shrink-0 transition-colors duration-300 ${bgClass}`}>
            ...
            <div className="w-full h-full flex items-stretch relative z-10">
                {item ? (
                    <>
                        {/* BOTÓN RESTAR (PDA Ergonomics) */}
                        <button 
                            onPointerDown={(e) => { e.preventDefault(); onDecrement(item); }} 
                            className="w-1/4 bg-black/10 active:bg-black/40 flex items-center justify-center border-r border-white/5"
                        >
                            <Minus className="w-12 h-12 text-white/50 active:text-white" />
                        </button>
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center px-4 text-center overflow-hidden">
                            <div className="mb-2 w-full">
                                ...
                                <h2 className="text-white font-black text-xs md:text-sm uppercase tracking-tight line-clamp-2 px-4 leading-tight italic">
                                    {item.name}
                                </h2>
                            </div>
                            
                            <div className="relative">
                                {/* PISO DE SEGURIDAD: Nunca mostrar menos de 0 en el display */}
                                <div className="text-[10rem] md:text-[12rem] font-black tabular-nums leading-none tracking-tighter drop-shadow-[0_15px_40px_rgba(0,0,0,0.6)] transform-gpu">
                                    {Math.max(0, item.totalQuantity)}
                                </div>
                                
                                {item.expectedQty !== undefined && item.expectedQty > 0 && (
                                    <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 bg-black/50 backdrop-blur-md border border-white/20 px-5 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] whitespace-nowrap shadow-xl">
                        ...
                        <button 
                            ...
                            className="w-1/4 bg-black/10 active:bg-black/40 flex items-center justify-center border-l border-white/5"
                        >
                            <Plus className="w-12 h-12 text-white/50 active:text-white" />
                        </button>
                    </>
                ) : (
            ...
            {/* FLASH DE CONFIRMACIÓN (Hardware Feedback) */}
            {feedback === 'success' && <div className="absolute inset-0 z-50 bg-white/10 pointer-events-none animate-pda-flash"></div>}
            
            <style>{`
                @keyframes pda-flash { 0% { opacity: 1; } 100% { opacity: 0; } }
                .animate-pda-flash { animation: pda-flash 0.15s cubic-bezier(0.4, 0, 0.2, 1) forwards; }
            `}</style>
        </div>
    );
});

            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-2xl bg-indigo-500/10 border border-indigo-500/20">
                  <div className="flex items-center gap-2 text-indigo-400 mb-1">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest">Conflictos</span>
                  </div>
                  <div className="text-2xl font-black text-white">{conflicts}</div>
                  <p className="text-[9px] text-slate-500 font-bold uppercase mt-1">Fusiones automáticas realizadas</p>
                </div>
                
                <div className={`p-4 rounded-2xl border ${incidents.length > 0 ? 'bg-rose-500/10 border-rose-500/20' : 'bg-emerald-500/10 border-emerald-500/20'}`}>
                  <div className={`flex items-center gap-2 mb-1 ${incidents.length > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
                    {incidents.length > 0 ? <AlertCircle className="w-4 h-4" /> : <CheckCircle2 className="w-4 h-4" />}
                    <span className="text-[10px] font-black uppercase tracking-widest">Incidentes</span>
                  </div>
                  <div className={`text-2xl font-black ${incidents.length > 0 ? 'text-rose-500' : 'text-emerald-500'}`}>
                    {incidents.length}
                  </div>
                  <p className="text-[9px] text-slate-500 font-bold uppercase mt-1">Errores de escritura pendientes</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between px-2">
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Historial de Incidentes</span>
                  {incidents.length > 0 && (
                    <button 

...
export const MassiveHeader: React.FC<Props> = memo(({ 
    ...
    // Handler robusto para prevenir ghost clicks en mobile
    const handleToolsClick = (e: React.MouseEvent | React.PointerEvent) => {
        e.preventDefault();
        e.stopPropagation();
        onOpenTools();
    };
    ...
    return (
        <header className="h-16 px-4 flex items-center justify-between border-b border-white/10 bg-slate-900 shadow-2xl shrink-0 z-[100] relative">
            ...
            <div className="flex-1 flex justify-center">
                <div className="bg-white/5 px-3 py-1 rounded-full border border-white/5">
                    <span className="text-[9px] font-black text-blue-500 uppercase tracking-[0.3em]">Auditoría_V4</span>
                </div>
            </div>
            ...
            <div className="flex items-center gap-2">
                ...
                <button 
                    onPointerDown={handleToolsClick}
                    className="w-12 h-12 flex items-center justify-center bg-white/10 rounded-xl border-2 border-white/10 active:bg-blue-600 active:border-blue-400 transition-all text-white shadow-xl"

...
export const InitializationService = {
    ...
    run: async (onStep: (step: InitStep) => void): Promise<void> => {
        // PRE-CALENTAMIENTO NEURONAL (No bloqueante)
        // Intentamos cargar el modelo local en segundo plano. Si ya está en caché, carga rápido.
        // Si no está y hay internet, se descarga. Si falla, no bloquea el arranque.
        localBrain.init().then(() => {
            console.log("[InitService] Motor Neural Local listo y en memoria.");
        }).catch(err => {
            console.warn("[InitService] Motor Neural pospuesto (se cargará bajo demanda).", err);
        });

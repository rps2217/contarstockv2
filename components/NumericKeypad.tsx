...
export const NumericKeypad: React.FC<NumericKeypadProps> = ({ 
  ...
  return (
    <div className={`fixed inset-0 z-[60] flex flex-col justify-end bg-black/40 backdrop-blur-sm animate-in fade-in duration-200 touch-manipulation`}>
      <div className="absolute inset-0" onClick={onClose}></div>
      <div className={`bg-slate-900 border-t border-slate-700 rounded-t-3xl shadow-2xl p-4 w-full max-w-lg mx-auto animate-in slide-in-from-bottom-full duration-300 pb-safe-area`}>
        ...
        <div className="flex justify-between items-center mb-6 px-2">
            ...
            <button onClick={onClose} className="p-2 text-slate-500 hover:text-slate-300 bg-slate-800 rounded-full active:bg-slate-700">
                <X className="w-6 h-6" />
            </button>
        </div>
        <div className="grid grid-cols-3 gap-3 select-none">
            {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
                <button key={num} onClick={() => handlePress(num.toString())} className="h-16 bg-slate-800 text-white text-2xl font-bold rounded-2xl shadow-sm border-b-4 border-slate-950 active:border-b-0 active:translate-y-1 transition-all active:bg-slate-700">{num}</button>
            ))}
            ...
            <button onClick={() => handlePress("0")} className="h-16 bg-slate-800 text-white text-2xl font-bold rounded-2xl shadow-sm border-b-4 border-slate-950 active:border-b-0 active:translate-y-1 transition-all active:bg-slate-700">0</button>
            <button onClick={() => onDelete()} className="h-16 bg-slate-800 text-red-400 rounded-2xl shadow-sm border-b-4 border-slate-950 active:border-b-0 active:translate-y-1 transition-all flex items-center justify-center active:bg-slate-700"><Delete className="w-8 h-8" /></button>
        </div>
      </div>
    </div>
  );

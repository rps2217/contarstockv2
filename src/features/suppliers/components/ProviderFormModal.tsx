      ...
      >
        <div className={`flex items-center justify-between p-6 border-b ${
          ...
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
              ...
              <Truck className={`w-5 h-5 ${theme === 'dark' ? 'text-brand-info' : 'text-indigo-600'}`} />
            </div>
            <div>
              <h2 className={`text-lg font-black uppercase ${theme === 'dark' ? 'text-white' : 'text-slate-800'}`}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className={`rounded-2xl p-5 border space-y-4 ${
            theme === 'dark' ? 'bg-brand-dark border-white/5' : 'bg-slate-50 border-slate-200'
          }`}>
            <h3 className={`text-sm font-black uppercase flex items-center gap-2 ${
              theme === 'dark' ? 'text-white' : 'text-slate-700'
            }`}>
              <ShieldAlert className={`w-4 h-4 ${theme === 'dark' ? 'text-brand-info' : 'text-indigo-500'}`} />
              Política de Vencimientos
            </h3>

            <label className={`flex items-center gap-3 cursor-pointer p-3 rounded-xl border transition-colors ${
              theme === 'dark' ? 'bg-brand-surface border-white/5 hover:border-brand-info' : 'bg-white border-slate-200 hover:border-indigo-300'
            }`}>
              <input
                type="checkbox"
              ...
              />
              <div>
                <p className={`text-sm font-bold ${theme === 'dark' ? 'text-white' : 'text-slate-800'}`}>Acepta Canje / Devolución</p>
          ...
          <div className="pt-4 flex gap-3">
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 font-bold rounded-xl transition-colors shadow-lg ${
                theme === 'dark' ? 'bg-brand-warning text-black hover:bg-brand-warning/90 shadow-brand-warning/10' : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-indigo-200'
              }`}
            >

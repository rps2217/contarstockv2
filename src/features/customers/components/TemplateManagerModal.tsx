        ...
        >
          ...
          <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
            ...
            <div className={`w-full md:w-1/2 border-b md:border-b-0 md:border-r overflow-y-auto p-4 space-y-3 ${
              theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' || theme === 'gray' ? 'border-white/10 bg-base/30' : 'border-slate-200 bg-slate-50/50'
            }`}>
                ...
                <div 
                  ...
                  className={`p-3 rounded-xl border transition-all cursor-pointer ${
                    editingTemplate?.id === template.id
                      ? (theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' || theme === 'gray' ? 'bg-blue-500/10 border-blue-500/50' : 'bg-blue-50 border-blue-200')
                      : (theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' || theme === 'gray' ? 'bg-white/5 border-white/10 hover:border-white/20' : 'bg-white border-slate-200 hover:border-slate-300')
                  }`}
                  onClick={() => handleEdit(template)}
                >
                  <div className="flex items-start justify-between gap-2">
                    <h4 className={`font-bold text-sm truncate ${theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' || theme === 'gray' ? 'text-white' : 'text-slate-900'}`}>
                      {template.name}
                    </h4>
                    <button
                      onClick={(e) => { e.stopPropagation(); handleDelete(template.id); }}
                      className={`p-1.5 rounded-lg transition-colors shrink-0 ${
                        theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' || theme === 'gray' ? 'hover:bg-rose-500/20 text-rose-400' : 'hover:bg-rose-100 text-rose-500'
                      }`}
                    >
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  </div>
                  <p className={`text-xs mt-1 line-clamp-2 ${theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' || theme === 'gray' ? 'text-muted' : 'text-slate-500'}`}>
                    {template.content}
                  </p>
                </div>
              ))}
              ...
              {templates.length === 0 && (
                <div className={`text-center py-8 text-sm ${theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' || theme === 'gray' ? 'text-slate-500' : 'text-muted'}`}>
              ...
              <form onSubmit={handleSave} className="space-y-4">
                ...
                <div>
                  ...
                  <div className={`mt-2 p-3 rounded-xl border text-xs ${
                    theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' || theme === 'gray' ? 'bg-blue-500/10 border-blue-500/20 text-blue-200' : 'bg-blue-50 border-blue-100 text-blue-800'
                  }`}>

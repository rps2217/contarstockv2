...
export const SyncManagerUI: React.FC = () => {
    ...
    return (
        <div className="flex flex-col h-[calc(100dvh-6rem)] md:h-screen bg-slate-50 text-slate-900 overflow-hidden font-sans page-transition">
            {/* Header Compacto */}
            <div className="bg-white border-b border-slate-200 px-4 py-4 flex items-center justify-between shrink-0 z-20 shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-lg font-black uppercase tracking-tight text-slate-900 leading-none">Nube</h1>
                        <div className="flex items-center gap-1.5 mt-1">
                            {navigator.onLine ? (
                                <span className="text-[9px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full flex items-center gap-1 uppercase tracking-widest border border-emerald-100"><Wifi className="w-2.5 h-2.5" /> Online</span>
                            ) : (
                                <span className="text-[9px] font-black text-rose-600 bg-rose-50 px-2 py-0.5 rounded-full flex items-center gap-1 uppercase tracking-widest border border-rose-100"><WifiOff className="w-2.5 h-2.5" /> Offline</span>
                            )}
                        </div>
                    </div>
                </div>
                <div className="flex gap-2">
                    ...
                    <button 
                        ...
                        className="p-3 bg-rose-50 text-rose-600 rounded-xl hover:bg-rose-100 active:scale-95 transition-all"
                        title="Reset Motor"
                    >
            ...
            <div className="flex-1 flex flex-col p-4 overflow-hidden">
                <div className="flex-1 overflow-y-auto no-scrollbar pb-2 min-h-[100px]">
                    {state.uiGroups.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-center opacity-40">
                            ...
                            <h2 className="text-lg font-black text-slate-900 uppercase tracking-tight">Sin Pendientes</h2>
                        ...
                        <div className="max-w-2xl mx-auto w-full space-y-3">
                            ...
                            {state.uiGroups.map(g => (
                                <div key={g.erpOrder} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex justify-between items-center transition-all">
                                    <div className="flex items-center gap-3 min-w-0">
                                        <div className={`p-2.5 rounded-xl shrink-0 ${g.uiStatus === 'success' ? 'bg-emerald-50 text-emerald-600' : (g.uiStatus === 'error' ? 'bg-rose-50 text-rose-600' : 'bg-blue-50 text-blue-600')}`}>
                                    ...
                                    <div className="pl-3 shrink-0">
                                        ...
                                        {g.uiStatus === 'error' && <div className="text-rose-600 text-[8px] font-black uppercase bg-rose-50 px-2 py-1 rounded">Error</div>}
                                        {g.uiStatus === 'idle' && <div className="w-2.5 h-2.5 bg-slate-100 border border-slate-200 rounded-full"></div>}
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                {/* Console Log Area - Compacto en móvil */}
                <div className="mt-2 bg-slate-900 rounded-2xl p-4 flex flex-col shadow-lg shrink-0 overflow-hidden border-t-4 border-slate-800">
            ...
            {/* Botón de Acción Principal - Pegado arriba de la nav */}
            <div className="p-4 bg-white border-t border-slate-100 shrink-0 z-30">
                <button 
                    ...
                    className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-slate-200 disabled:text-slate-400 text-white font-black py-4 rounded-2xl shadow-xl flex items-center justify-center gap-3 transition-all active:scale-95 uppercase tracking-widest text-xs"
                >
                    {state.isProcessing ? (
                        <>

...
const MassiveBlindView: React.FC = () => {
    ...
    return (
        <div className={`h-screen w-full transition-colors duration-100 flex flex-col font-mono select-none overflow-hidden ${isFlash ? 'bg-emerald-500' : 'bg-black'}`}>
            ...
            <header className="h-14 px-4 flex items-center justify-between border-b border-white/10 shrink-0 bg-black/80 backdrop-blur-md z-50">
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                        <span className="text-[8px] font-black text-blue-500 uppercase tracking-widest leading-none">MODO RÁFAGA</span>
                        <span className="text-xs text-white font-bold tracking-tighter uppercase">{batchId}</span>
                    </div>
                </div>
                
                <div className="flex items-center gap-4">
                    <div className="bg-white/5 px-4 py-1.5 rounded-xl border border-white/10 flex items-center gap-2">
            ...
            <div className="h-[40vh] md:h-[45vh] relative shrink-0 border-b-4 border-white/5 bg-slate-900 overflow-hidden">
                {isCameraActive ? (
                    <CameraScanner 
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center p-8 bg-gradient-to-b from-slate-900 to-black">
                        <div className="text-[10px] font-black text-white/20 uppercase tracking-[0.6em] mb-4">Sensor HID Listo</div>
                        <div className="relative">
                            <ScanLine className="w-24 h-24 text-blue-500/20 animate-pulse" />
                        ...
                        <button 
                            ...
                            className="mt-8 bg-blue-600 text-white px-8 py-3 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl active:scale-95 transition-all"
                        >
                            Activar Cámara
                        </button>
                    </div>
                ...
                <div className="max-w-2xl mx-auto space-y-2">
                    {items && items.length > 0 ? (
                        items.map((item) => (
                            <div 
                                key={item.barcode} 
                                className="bg-white/[0.03] border border-white/5 rounded-2xl p-4 flex items-center justify-between animate-in slide-in-from-right-4"
                            >
                                <div className="flex-1 min-w-0 pr-4">
                                    <h3 className="text-white font-black text-sm uppercase truncate leading-tight tracking-tight">
                                        {item.name}
                                    </h3>
                                    <div className="flex items-center gap-2 mt-1">
                                        <span className="text-[10px] font-bold text-blue-400 tracking-wider bg-blue-500/10 px-2 py-0.5 rounded border border-blue-500/20">
                                            {item.barcode}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-1.5">
                                    <button 
                                        ...
                                        className="w-12 h-14 bg-white/5 text-rose-500 flex items-center justify-center rounded-xl active:bg-rose-500 active:text-white transition-all border border-white/5 shadow-inner"
                                    >
                                        <Minus className="w-6 h-6 stroke-[3px]" />
                                    </button>
                                    
                                    <div className="w-20 h-14 bg-white/5 flex items-center justify-center rounded-xl border border-white/10">
                                    ...
                                    <button 
                                        onClick={() => updateItemQty(item.barcode, 1)}
                                        className="w-12 h-14 bg-blue-600 text-white flex items-center justify-center rounded-xl active:scale-90 transition-all shadow-lg"
                                    >
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-black/90 backdrop-blur-xl border-t border-white/5 px-6 py-4 flex items-center justify-between z-50">
                <div className="flex items-center gap-4">
                    ...
                    <span className="text-[9px] font-black text-white/40 uppercase tracking-[0.3em]">Scanner Hardware: Conectado</span>
                </div>

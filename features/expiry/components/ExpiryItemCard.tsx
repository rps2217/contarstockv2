...
export const ExpiryItemCard: React.FC<ExpiryItemCardProps> = ({
  ...
  return (
    <motion.div
      ...
      className={`border rounded-2xl p-3 flex items-center justify-between group cursor-pointer transition-all ${
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            {item.batch && item.batch !== 'N/A' && (
              <span className="text-[9px] font-black bg-emerald-500/20 px-1.5 py-0.5 rounded text-emerald-400 uppercase tracking-widest border border-emerald-500/20">
                LOTE: {item.batch}
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-4">
            <h3 className={`text-sm font-black uppercase tracking-tighter italic truncate max-w-[300px] ${
          ...
          <div className={`mt-2 w-full h-1 rounded-full overflow-hidden ${
            ...
            <motion.div 
              ...
              className={`h-full rounded-full ${
                item.status === 'expired' ? 'bg-rose-500 shadow-[0_0_10px_rgba(244,63,94,0.5)]' : 
                item.status === 'critical' ? 'bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.5)]' :
                item.status === 'withdrawal' ? 'bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]' :
                'bg-emerald-500'
              }`}
            />
          </div>

...
// Tipo extendido para supplier con hasCanje
export type SupplierWithCanje = Provider & { hasCanje?: boolean };
    ...
    <div className="flex items-center gap-2">
      <span
        className={cn(
          ...
          (supplier as SupplierWithCanje).hasCanje
            ? 'bg-emerald-500/20 text-emerald-500'
            : 'bg-muted/20 text-muted'
        )}
      >
        {(supplier as SupplierWithCanje).hasCanje ? 'Con Canje' : 'Sin Canje'}
      </span>
          ...
          <div className="flex flex-col gap-2">
            {filtered.length === 0 ? (
              <EmptyState
                ...
                action={
                  !searchQuery
                    ? { label: 'Agregar proveedor', onClick: actions.openCreate }
                    : undefined
                }
              />
            ) : (

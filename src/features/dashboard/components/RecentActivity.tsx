...
export const RecentActivity: React.FC<RecentActivityProps> = memo(
  ({ title, items, onItemClick, isDark = true, maxItems = 5 }) => {
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="w-10 h-10 rounded-full bg-elevated flex items-center justify-center">
                    <span
                      className={cn('text-xs font-bold', getIconColorClass(item.iconColor, isDark))}
                    >
                ...
                {item.count !== undefined && (
                  <div className="text-right">
                    <p className="text-sm font-semibold text-secondary">{item.count}</p>

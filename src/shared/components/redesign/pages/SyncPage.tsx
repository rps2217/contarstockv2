...
// Tipos locales
type SyncQueueItem = { id?: number; tableName: string; operation: string };
type SyncLogItem = {
  id?: number;
  status: string;
  action?: string;
  ...
  const getStatusConfig = () => {
    if (isSyncing) {
      return {
        ...
        buttonBg: 'bg-blue-500/50 cursor-not-allowed',
      };
    }
    if (isSupabaseConnected) {
      return {
        ...
        title: 'Conectado',
        subtitle: `Última sync: ${lastSyncDisplay}${latencyMs ? ` • ${latencyMs}ms` : ''}`,
        ...
        <div className="max-w-3xl mx-auto flex flex-col gap-6">
          ...
          >
            ...
            <div className="flex items-center gap-4 relative z-10">
              ...
              >
                {isSyncing ? (
                  <Loader2 className={cn('w-7 h-7', status.iconColor, 'animate-spin')} />
                ) : isSupabaseConnected ? (
                  <Wifi className={cn('w-7 h-7', status.iconColor)} />
                ) : (
                  <WifiOff className={cn('w-7 h-7', status.iconColor)} />
                )}
              </div>
              <div>
                ...
                {isSyncing && syncDuration && (
                  <p className="text-xs text-muted mt-1">
                    Duración: {formatDuration(syncDuration)}
                  </p>
                )}
              </div>
            </div>
            <button
              ...
              className={cn(
                'w-full sm:w-auto font-bold px-6 py-3 rounded-xl flex items-center justify-center gap-2 transition-all relative z-10 shadow-lg',
                status.buttonBg,
                !isSupabaseConnected && 'opacity-50'
              )}
            >
          ...
          <AnimatePresence>
            {isSyncing && (
              <motion.div
                ...
                className="bg-surface border border-subtle rounded-2xl p-4"
              >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              },
              {
                ...
                bg: 'bg-purple-500/10',
                label: 'Última sync',
                value: lastSyncTime ? formatTimeAgo(lastSyncTime) : '--',
              },
              {
                icon: TrendingUp,
          ...
          {syncQueue && syncQueue.length > 0 && (
            ...
            >
              ...
              <div className="space-y-2 max-h-40 overflow-y-auto">
                {syncQueue.slice(0, 10).map((item: SyncQueueItem) => (
                  <div
                    key={item.id}
                  ...
                  >
                    ...
                    <span className="text-xs text-muted font-mono">
                      {item.operation || 'pending'}
                    </span>
                  </div>
                ))}
                {syncQueue.length > 10 && (
                  <p className="text-xs text-muted text-center">+{syncQueue.length - 10} más...</p>
                )}
              </div>
            </motion.div>
          )}
            ...
            <div className="bg-surface border border-subtle rounded-2xl overflow-hidden">
              {!syncLogs || syncLogs.length === 0 ? (
                <div className="p-8 text-center">
                  ...
                  <p className="text-sm text-muted">No hay historial</p>
                </div>
              ) : (
                syncLogs.slice(0, 15).map((log: SyncLogItem, idx: number) => (
                  <motion.div
                    key={log.id}
                    initial={{ opacity: 0, x: -10 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: idx * 0.05 }}
                    className="flex items-start gap-4 p-4 border-b border-subtle last:border-0 hover:bg-base/50"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="text-sm font-medium text-primary">
                          {log.action || log.tableName || 'Sync'}
                        </p>
                        <span className="text-xs text-muted">•</span>
                        <span className="text-xs text-muted">
                          {log.timestamp ? formatTimeAgo(log.timestamp) : 'Ahora'}
                        </span>
                      </div>
                      {log.errorMessage && (
                        <p className="text-xs text-rose-500 mt-1">{log.errorMessage}</p>
                      )}
                    </div>
                  </motion.div>
                ))
          ...
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
            {[
              ...
              <div
                key={stat.label}
                className="bg-surface border border-subtle rounded-xl p-3 text-center"
              >

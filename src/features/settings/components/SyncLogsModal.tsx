...
export const SyncLogsModal: React.FC<Props> = ({ isOpen, onClose, theme = 'dark' }) => {
  ...
  const getDetailBg = (status: string) => {
    if (status === 'success') {
      return isHighContrast
        ? 'bg-yellow-950/30 text-green-300 border-green-500/20'
        : isLight
          ? 'bg-slate-100 text-emerald-700 border-emerald-200'
          : 'bg-black/80 text-emerald-300 border-emerald-500/20';
    }
    return isHighContrast
      ? 'bg-yellow-950/30 text-red-300 border-red-500/20'
      : isLight
        ? 'bg-slate-100 text-rose-700 border-rose-200'
        : 'bg-black/80 text-rose-300 border-rose-500/20';
  };
  ...
  return (
    <div
      className={`fixed inset-0 z-[300] backdrop-blur-md flex items-center justify-center p-4 md:p-8 animate-in fade-in duration-200 ${overlayBg}`}
    >
      <motion.div
      ...
      >
        {/* HEADER */}
        <div
          className={`p-6 border-b flex items-center justify-between backdrop-blur-xl ${headerBg} ${headerBorder}`}
        >
          <div className="flex items-center gap-4">
            <div
              className={`p-3 rounded-2xl shadow-lg ${isHighContrast ? 'bg-yellow-400 text-black' : isLight ? 'bg-indigo-600 shadow-indigo-900/40 text-white' : 'bg-indigo-600 shadow-lg shadow-indigo-900/40 text-white'}`}
            >
              <Activity className="w-6 h-6" />
            </div>
            <div>
              <h3
                className={`text-xl font-black uppercase italic tracking-tighter leading-none ${headerText}`}
              >
                Centro de Diagnóstico
              </h3>
              <p className={`text-[10px] font-bold uppercase tracking-widest mt-1 ${subtitleText}`}>
                Logs de Red y Sincronización en Tiempo Real
              </p>
            </div>
          </div>
        ...
        {/* TOOLBAR */}
        <div
          className={`p-4 flex flex-col md:flex-row gap-4 items-center justify-between ${toolbarBg} ${headerBorder}`}
        >
          ...
          <div className="flex-1 max-w-md relative">
            <Search
              className={`absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 ${subtitleText}`}
            />
            <input
              value={search}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
          {isLoading ? (
            ...
            <div
              className={`h-full flex items-center justify-center border-4 border-dashed rounded-[2rem] ${emptyStateBg}`}
            >
              <div className="text-center space-y-4">
                <div
                  className={`w-20 h-20 rounded-3xl flex items-center justify-center mx-auto mb-6 ${isHighContrast ? 'bg-yellow-900/20' : isLight ? 'bg-slate-100' : 'bg-white/5'}`}
                >
                  <Database className={`w-10 h-10 ${emptyText}`} />
                </div>
                <h4 className={`text-lg font-black uppercase italic ${emptyText}`}>
                  Sin actividad registrada
                </h4>
                <p
                  className={`text-[10px] uppercase font-bold tracking-widest max-w-xs mx-auto ${emptyText}`}
                >
            ...
            logs.map(log => {
              ...
              return (
                ...
                >
                  <div
                    onClick={() =>
                      setExpandedLogId(expandedLogId === log.id ? null : log.id || null)
                    }
                    className="p-4 flex items-center gap-4 cursor-pointer"
                  >
                    <div
                      className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${status.bg}`}
                    >
                      {status.icon}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span
                          className={`text-[10px] font-black uppercase tracking-tighter ${headerText}`}
                        >
                          {log.action}
                        </span>
                        <span className={`text-[8px] font-bold uppercase ${subtitleText}`}>•</span>
                        <span
                          className={`text-[10px] font-bold uppercase tracking-widest ${isHighContrast ? 'text-yellow-400' : isLight ? 'text-indigo-600' : 'text-indigo-400'}`}
                        >
                          {log.tableName}
                        </span>
                      </div>
                      <div className={`flex items-center gap-2 text-[9px] ${subtitleText}`}>
                        ...
                        {log.errorMessage && (
                          <>
                            <span
                              className={`mx-1 ${isHighContrast ? 'text-yellow-700' : isLight ? 'text-secondary' : 'text-slate-700'}`}
                            >
                              |
                            </span>
                            <span
                              className={`font-bold uppercase truncate ${isHighContrast ? 'text-red-400' : 'text-rose-400'}`}
                            >
                              {log.errorMessage}
                            </span>
                          </>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div className="space-y-3">
                            <div className="flex justify-between items-center px-1">
                              <div
                                className={`flex items-center gap-2 ${isHighContrast ? 'text-yellow-400' : isLight ? 'text-indigo-600' : 'text-indigo-400'}`}
                              >
                              ...
                              <button
                                ...
                                className={`p-2 rounded-lg transition-colors ${isHighContrast ? 'bg-yellow-900/20 text-yellow-400 hover:text-yellow-300' : isLight ? 'bg-slate-100 text-muted hover:text-slate-600' : 'bg-white/5 text-muted hover:text-white'}`}
                              >
                                <Copy className="w-4 h-4" />
                              </button>
                            </div>
                            <pre
                              className={`rounded-2xl p-4 text-[10px] font-mono overflow-x-auto max-h-60 custom-scrollbar border ${isHighContrast ? 'bg-yellow-950 border-yellow-400/30 text-yellow-300' : isLight ? 'bg-slate-200 border-slate-200 text-indigo-700' : 'bg-black/80 border-white/5 text-blue-300'}`}
                            >
                              {(() => {
                                const cache = new Set();
                                return JSON.stringify(
                          ...
                          <div className="space-y-3">
                            <div className="flex justify-between items-center px-1">
                              <div
                                className={`flex items-center gap-2 ${isHighContrast ? 'text-green-400' : isLight ? 'text-emerald-600' : 'text-emerald-400'}`}
                              >
                            ...
                            <pre
                              className={`rounded-2xl p-4 text-[10px] font-mono overflow-x-auto max-h-60 custom-scrollbar border ${getDetailBg(log.status)}`}
                            >
        ...
        <div className={`p-4 border-t text-center ${footerBg} ${headerBorder}`}>
          <p className={`text-[9px] font-bold uppercase tracking-widest ${emptyText}`}>
            Sugerencia: Si ves errores de "Missing or insufficient permissions", verifica las reglas
            de seguridad en la consola de Firebase.
          </p>
        </div>
      </motion.div>
    </div>

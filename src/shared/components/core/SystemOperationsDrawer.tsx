...
export const SystemOperationsDrawer: React.FC = () => {
  ...
  const usagePercent = state.storageUsage
    ? Math.min(100, (state.storageUsage.used / state.storageUsage.quota) * 100)
    : 0;
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[120] flex justify-end">
        ...
        >
              ...
              <div className="grid grid-cols-2 gap-2.5">
                ...
                <div className="p-3.5 rounded-xl border border-slate-200 dark:border-white/5 bg-slate-50/50 dark:bg-white/2">
                  ...
                  <div className="text-sm font-black mt-1.5 truncate">
                    {isOnline && isSupabaseConnected && latencyMs !== null
                      ? `${latencyMs} ms`
                      : 'N/A'}
                  </div>
                </div>
              ...
              <div className="p-4 rounded-xl border border-slate-200 dark:border-white/5 bg-slate-50/50 dark:bg-white/2 space-y-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="text-xs font-black uppercase block tracking-tight">
                      Capacidad IndexedDB
                    </span>
                    <span className="text-[9px] font-bold text-muted uppercase tracking-widest block">
                      Uso actual de almacenamiento persistente
                    </span>
                  </div>
                </div>
              ...
              <div className="p-4 rounded-xl border border-indigo-500/10 dark:border-indigo-500/10 bg-indigo-500/2 dark:bg-indigo-500/2 space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <span className="text-[9px] font-bold text-muted uppercase tracking-widest block mt-0.5">
                ...
                <div className="flex gap-2 border-t border-slate-200 dark:border-white/5 pt-3">
                  <button
                    onClick={() => {
                      if (
                        confirm(
                          '¿Desea forzar el reemplazo de la base de datos remota con su datos locales?'
                        )
                      ) {
              ...
              <div className="p-4 rounded-xl border border-slate-200 dark:border-white/5 bg-slate-50/50 dark:bg-white/2 space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <span className="text-[9px] font-bold text-muted uppercase tracking-widest block mt-0.5">
                      {isModelReady
                        ? 'Modelo Transformers cargado en navegador'
                        : 'Instale el motor IA de búsqueda inteligente'}
                    </span>
                  </div>
                </div>
              ...
              <div className="p-4 rounded-xl border border-rose-500/10 bg-rose-500/5 space-y-3">
                <button
                  onClick={() =>
                    updateSetting(
                      'theme',
                      settings.theme === 'high-contrast' ? 'dark' : 'high-contrast'
                    )
                  }
                ...
                <button
                  onClick={async () => {
                    if (
                      window.confirm(
                        '¿Estás 100% seguro de que deseas VACIAR todo el catálogo de PRODUCTOS y PROVEEDORES? Esto es irreversible localmente (aunque podrías recuperar desde la nube si hiciste respaldo).'
                      )
                    ) {
                      ...
                      try {
                        ...
                        const { ProviderRepository } =
                          await import('@/repositories/ProviderRepository');
                ...
                <button
                  onClick={async () => {
                    if (
                      window.confirm(
                        '¿Estás seguro de que deseas VACIAR todos los REGISTROS DE VENCIMIENTO? Esta acción eliminará todo tu trabajo local de mermas y canjes.'
                      )
                    ) {
                      const tId = toast.loading('Vaciando registros de vencimiento...');
                      try {
                        const { expiryRepository } =
                          await import('@/repositories/ExpiryRepository');
                        await expiryRepository.clear();
                        toast.success('Registros de vencimiento vaciados correctamente.', {
                          id: tId,
                        });
                        window.location.reload();
                      } catch (error: any) {
          ...
          <div className="p-4 border-t border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-surface/40 text-center flex items-center justify-center gap-2 text-[10px] font-bold text-muted uppercase tracking-widest select-none">

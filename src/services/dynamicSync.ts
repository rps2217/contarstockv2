...
export const dynamicSyncService = {
  ...
  async syncAllPending(
    ...
    let query = db.dynamic_data.where('syncStatus').anyOf(['pending', 'error', 'pending_delete']); // Incluir eliminaciones pendientes

    if (tableNameFilter) {
      query = db.dynamic_data.where('[tableName+syncStatus]').anyOf([
    ...
    const sortedTableNames = Object.keys(groups).sort(
      (a, b) => (tablePriority[b] || 0) - (tablePriority[a] || 0)
    );
    ...
    for (const tableName of sortedTableNames) {
      ...
      if (onProgress)
        onProgress(
          `Sincronizando ${records.length} registros de ${tableName}... (Prioridad ${tablePriority[tableName] || 0})`
        );
      ...
      for (let i = 0; i < records.length; i += BATCH_SIZE) {
        ...
        try {
          ...
          if (config?.mappings) {
            if (
              tableName === config.inventoryRegistryTableName ||
              tableName === config.expiryTableName
            ) {
          ...
          if (result?.success) {
            ...
            if (onProgress)
              onProgress(
                `✓ ${tableName}: ${batchRecords.length} sincronizados (Lote ${Math.floor(i / BATCH_SIZE) + 1}).`
              );
          } else {
          ...
          logger.error(
            'DYNAMIC_SYNC_FAIL',
            `Error sincronizando lote de ${tableName}: ${(error as Error).message}`
          );

          // NOTIFICAR INCIDENTE (Joyeria UX)
          useSyncStore.getState().addIncident(tableName, (error as Error).message);
      ...
      await db.transaction('rw', db.dynamic_data, async () => {
        ...
        for (const rawRow of batch) {
          ...
          const remoteId = String(
            remoteRow[idColumn] ||
              remoteRow['ID'] ||
              remoteRow['ID_REGISTRO'] ||
              remoteRow['CLAVE_UNICA'] ||
              ''
          );

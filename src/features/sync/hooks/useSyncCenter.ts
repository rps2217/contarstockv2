...
export function useSyncCenter(): UseSyncCenterReturn {
  ...
  const { incidents, lastSyncTime, isSupabaseConnected, syncError, clearIncidents, conflicts } =
    useSyncStore();
  ...
  const stats = useLiveQuery(async () => {
    ...
    for (const [key, meta] of Object.entries(syncRegistry)) {
      ...
      if (meta.filterField && meta.filterValue) {
        query = localTable
          .where(meta.filterField)
          .equals(meta.filterValue) as unknown as GenericDexieTable;
      }
      ...
      const pendingDeleteRecords = await query
        .filter((item: Record<string, unknown>) => {
          const status = item.syncStatus as SyncStatus | undefined;
  ...
  const totalPending = stats
    ? Object.values(stats).reduce((acc, curr) => acc + curr.pending, 0)
    : 0;
  ...
  const handleSingleTableSync = useCallback(
    async (key: string) => {
      ...
      try {
        ...
        if (res.success) {
          ...
          const pull = (res.pullRes?.added || 0) + (res.pullRes?.updated || 0);
          addToast(
            `Sincronizado ${key.toUpperCase()}: Subidos ${push} Descargados ${pull}`,
            'success'
          );
        } else {

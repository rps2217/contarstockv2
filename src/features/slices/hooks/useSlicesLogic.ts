...
interface UseSlicesLogicReturn {
  ...
  editRow: (rowId: string, rowData: any) => Promise<void>;
...
export function useSlicesLogic(): UseSlicesLogicReturn {
  ...
  const sliceData = useLiveQuery(async () => {
    ...
    try {
      const rawData: any[] = await tableInstance.toArray();
  ...
  const filteredRows = useMemo(() => {
    ...
    return sliceData.filter((row: any) =>
      Object.values(row).some(val => {
        if (typeof val === 'object' && val !== null) {
          return Object.values(val).some((sub: any) => 
  ...
  // Crear slice
  const createSlice = useCallback((sliceData: Omit<AppSheetSlice, 'id' | 'isSystem'>) => {
  ...
  // Actualizar slice
  const updateSlice = useCallback((updatedSlice: AppSheetSlice) => {
  ...
  // Editar fila
  const editRow = useCallback(async (rowId: string, rowData: any) => {

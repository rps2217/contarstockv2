    ...
    return (
        <div style={style} className="px-4 py-2">
            <div 
                ...
                className={`bg-white dark:bg-slate-900 border-4 rounded-[2rem] h-full flex items-center px-6 gap-5 transition-all active:scale-[0.98] shadow-md relative overflow-hidden group ${isCertified ? 'border-emerald-500 bg-emerald-50/30' : (hasIssues ? 'border-rose-500' : 'border-black dark:border-white/10')}`}
            >
                ...
                <div className="flex-1 min-w-0 z-10 py-2">
                    <div className="flex items-center gap-3 mb-2">
                         <span className="text-[9px] font-black text-slate-500 bg-slate-100 dark:bg-white/5 px-2 py-0.5 rounded uppercase tracking-widest border border-slate-200 dark:border-white/10">
                    ...
                    <h3 className="text-2xl font-black text-black dark:text-white uppercase truncate tracking-tight leading-tight mb-1">
                        {session.erpOrder}
                    </h3>
                    <div className="flex items-center gap-2 text-[10px] font-bold text-blue-700 dark:text-blue-400 uppercase truncate">
                        <Truck className="w-3.5 h-3.5" /> <span className="opacity-80 tracking-widest">{session.logisticsLabel}</span>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4 shrink-0 z-10">
                    <div className="text-right">
                        ...
                        <div className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mt-1">Units</div>
                    </div>
                    
                    <button 
                        onClick={(e) => { e.stopPropagation(); onMenuToggle(e, session.id); }}
                        className="p-3 bg-slate-100 dark:bg-white/5 border-2 border-slate-200 dark:border-white/10 rounded-xl active:bg-slate-200 transition-colors"
                    >
                    ...
                    {activeMenuId === session.id && (
                        <>
                            ...
                            <div className="absolute right-4 top-14 w-64 bg-white dark:bg-slate-800 border-4 border-black dark:border-white/10 rounded-2xl shadow-2xl z-50 overflow-hidden animate-in zoom-in-95">
                                <button 

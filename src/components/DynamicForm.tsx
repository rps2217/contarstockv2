                ...
                <button
                  ...
                  className={`h-12 rounded-xl text-sm font-black transition-all border ${
                    values[key] === opt
                      ...
                      : (theme as unknown) === 'dark' || (theme as unknown) === 'night' || (theme as unknown) === 'high-contrast' || (theme as unknown) === 'appsheet-dark' || (theme as unknown) === 'gray' ? 'bg-white/5 border-white/10 text-muted' : 'bg-slate-50 border-slate-200 text-slate-600'
                  }`}
                >
                  {opt}
                </button>
          ...
          <button
            ...
            className={`flex items-center justify-between p-4 rounded-xl border transition-all ${
              values[key] 
                ...
                : (theme as unknown) === 'dark' || (theme as unknown) === 'night' || (theme as unknown) === 'high-contrast' || (theme as unknown) === 'appsheet-dark' || (theme as unknown) === 'gray' ? 'border-white/5 bg-white/5' : 'border-slate-200 bg-slate-50'
            }`}
          >
            <span className={`text-xs font-black uppercase tracking-widest ${(theme as unknown) === 'dark' || (theme as unknown) === 'night' || (theme as unknown) === 'high-contrast' || (theme as unknown) === 'appsheet-dark' || (theme as unknown) === 'gray' ? 'text-white' : 'text-slate-900'}`}>{col.label}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {Object.entries(schema.columns).filter(([_, col]) => col.visible !== false).map(([key, col]) => (
          <div key={key} className="space-y-2">
            <label className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-2 ${(theme as unknown) === 'dark' || (theme as unknown) === 'night' || (theme as unknown) === 'high-contrast' || (theme as unknown) === 'appsheet-dark' || (theme as unknown) === 'gray' ? 'text-slate-500' : 'text-muted'}`}>
              {col.label}
        ...
        {onCancel && (
          <button
            ...
            className={`flex-1 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${
              (theme as unknown) === 'dark' || (theme as unknown) === 'night' || (theme as unknown) === 'high-contrast' || (theme as unknown) === 'appsheet-dark' || (theme as unknown) === 'gray' ? 'bg-white/5 border-white/10 hover:bg-white/10' : 'bg-slate-100 border-slate-200 hover:bg-slate-200'
            }`}
          >
            Cancelar
          </button>

...
// Tipo genérico para filas de datos
...
export const SlicePreview: React.FC<Props> = ({
  ...
  const theadText = isLight
    ? 'text-slate-600'
    : isHighContrast
      ? 'text-yellow-400'
      : 'text-slate-500';
  const rowDivider = isLight
    ? 'divide-slate-200'
    : isHighContrast
      ? 'divide-yellow-400/20'
      : 'divide-slate-900';
  const rowHover = isLight
    ...
    : isHighContrast
      ...
      : 'hover:bg-surface/10';
  const cellText = isLight
    ? 'text-slate-700'
    : isHighContrast
      ? 'text-yellow-300'
      : 'text-secondary';

  const handleEditRow = (rowId: string, rowData: DataRow) => {
    ...
    window.dispatchEvent(
      new CustomEvent('slice-edit-row', {
  ...
  const handleDeleteRow = (rowId: string) => {
    ...
    window.dispatchEvent(
      new CustomEvent('slice-delete-row', {
        detail: { rowId },
      })
    );
  };

  const getStatusBadge = (col: string, val: unknown) => {
    const displayStr = typeof val === 'object' ? JSON.stringify(val) : String(val || '-');

    if (col === 'syncStatus' || col === 'status') {
      let badgeStyle = '';
      if (['synced', 'completed', 'active'].includes(String(val))) {
        badgeStyle = isLight
          ? 'bg-emerald-50 text-emerald-600 border-emerald-200'
          : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20';
      } else if (['pending', 'pending_delete'].includes(String(val))) {
        badgeStyle = isLight
          ? 'bg-blue-50 text-blue-600 border-blue-200'
          : 'bg-blue-500/10 text-blue-400 border-blue-500/20';
      } else if (String(val) === 'error') {
        badgeStyle = isLight
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className={`animate-pulse ${isLight ? 'text-muted' : 'text-slate-500'}`}>
          Cargando datos...
        </div>
      </div>
    );
  ...
  return (
    <div className={`overflow-x-auto rounded-2xl border ${tableBorder} ${tableBg}`}>
      <table className="w-full text-left font-mono text-[11px] leading-relaxed border-collapse">
        <thead>
          <tr
            className={`border-b ${tableBorder} ${theadBg} ${theadText} font-extrabold uppercase`}
          >
            {activeSlice.selectedColumns.map(col => (
              <th key={col} className="p-3.5 tracking-wider">
                {col}
              </th>
            ))}
        ...
        <tbody className={`divide-y ${rowDivider}`}>
          {filteredRows.length === 0 ? (
            <tr>
              <td
                colSpan={activeSlice.selectedColumns.length + 1}
                className={`p-8 text-center font-sans ${isLight ? 'text-muted' : 'text-slate-500'}`}
              >
                <BookOpen
                  className={`w-10 h-10 mx-auto mb-3 ${isLight ? 'text-secondary' : 'text-slate-800'}`}
                />
                Ningún registro cumple con los criterios de este Slice o la búsqueda actual.
              </td>
            </tr>
          ) : (
            filteredRows.map((rowArr: DataRow, index) => {
              const rowId = String(rowArr.id || rowArr.barcode || index);

...
export const useHammerLogic = (batchId: string) => {
  ...
  const dbItems = useLiveQuery(async () => {
    ...
    const uniqueBarcodes = Array.from(
      new Set([...rawScans.map(s => s.barcode), ...manifests.map(m => m.barcode)])
    );
    const products =
      uniqueBarcodes.length > 0
        ? await Promise.all(uniqueBarcodes.map(b => productRepository.getById(b)))
        : [];
    ...
    const results = Array.from(aggregation.values()).sort(
  ...
  const registerScan = useCallback(
    async (code: string, qtyOverride?: number) => {
      ...
      if (isManualEdit) {
        try {
          ...
          await HammerDbRepository.updateScanQuantity(
            batchId,
            clean,
            finalQty,
            locationRef.current
          ...
          if (autoSyncRef.current) {
            pushScansToCloud(batchId).catch(err => {
              logger.warn('HammerLogic', 'AutoSync manual edit push failed', {
                error: String(err),
              });
            });
          }
        ...
        if (barcode === 'ALL') {
          ...
          if (autoSyncRef.current) {
            pushScansToCloud(batchId).catch(err => {
              logger.warn(
                'HammerLogic',
                'Background sync failed after undo',

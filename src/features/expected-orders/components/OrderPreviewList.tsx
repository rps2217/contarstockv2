    ...
    >
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <span className={`text-[10px] font-mono leading-none tracking-wide block mt-0.5 ${isDark ? 'text-slate-500' : 'text-muted'}`}>
      ...
      <div className="flex items-center gap-2 shrink-0">
        <div className="text-right">
          ...
          <span className={`text-[9px] font-black uppercase tracking-wider block ${isDark ? 'text-slate-500' : 'text-muted'}`}>
            Bultos Esperados
          </span>
        </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 flex flex-col h-[480px]">
        <div className={`p-4 border-b flex items-center justify-between shrink-0 ${
          isDark ? 'bg-surface/60 border-white/5' : 'bg-white border-slate-200/80'
        } rounded-t-3xl`}>
        ...
        <div className={`flex-1 overflow-hidden relative border-x border-b ${
          isDark ? 'bg-base border-white/5' : 'bg-white border-slate-200/80 shadow-sm'
        } rounded-b-3xl`}>
          <VirtualList
            items={previewItems}
            itemHeight={55}
            ...
            emptyState={
              <div className="h-full w-full flex flex-col items-center justify-center p-8 text-center">
                <AlertTriangle className="w-8 h-8 text-muted mb-2" />
                <span className="text-xs font-black text-slate-500 uppercase tracking-widest block">Ningún SKU válido mapeado</span>
                <p className="text-[10px] text-muted max-w-xs leading-normal mt-1 block">
      ...
      <div className="lg:col-span-1 space-y-6">
        <div className={`p-6 md:p-8 rounded-[2rem] border ${
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className={`p-4 rounded-2xl border ${isDark ? 'bg-base border-white/5' : 'bg-slate-50 border-slate-100'}`}>
              <span className={`text-[9px] font-black uppercase tracking-wider block ${isDark ? 'text-slate-500' : 'text-muted'}`}>Total SKUs</span>
              <span className={`text-2xl font-black block mt-1 tracking-tight ${isDark ? 'text-white' : 'text-slate-800'}`}>
                {previewStats.skuCount}
              </span>
            </div>
            <div className={`p-4 rounded-2xl border ${isDark ? 'bg-base border-white/5' : 'bg-slate-50 border-slate-100'}`}>
              <span className={`text-[9px] font-black uppercase tracking-wider block ${isDark ? 'text-slate-500' : 'text-muted'}`}>Total Unidades</span>
          ...
          <div className={`p-4 rounded-2xl border space-y-2.5 ${isDark ? 'bg-base border-white/5 text-muted' : 'bg-slate-50 border-slate-100 text-slate-600'}`}>

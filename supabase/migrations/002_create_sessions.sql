-- =============================================================================
-- Crear tabla SESSIONS para sincronización de sesiones de conteo
-- =============================================================================
...
-- Política para usuarios autenticados: pueden ver todas las sesiones
CREATE POLICY "Authenticated users can view all sessions"
    ON sessions FOR SELECT
    TO authenticated
    USING (true);

-- Política para usuarios autenticados: pueden insertar sesiones
CREATE POLICY "Authenticated users can insert sessions"
    ON sessions FOR INSERT
    TO authenticated
    WITH CHECK (true);

-- Política para usuarios autenticados: pueden actualizar sesiones
CREATE POLICY "Authenticated users can update sessions"
    ON sessions FOR UPDATE
    TO authenticated
    USING (true);

-- Política para usuarios autenticados: pueden eliminar sesiones
...
COMMENT ON COLUMN sessions.id IS 'UUID único de la sesión';
COMMENT ON COLUMN sessions.status IS 'Estado: active, paused, completed';
COMMENT ON COLUMN sessions.erp_order IS 'Número de orden ERP';
COMMENT ON COLUMN sessions.logistics_label IS 'Etiqueta de logística';
COMMENT ON COLUMN sessions.session_type IS 'Tipo: standard, blind, theoretical';
COMMENT ON COLUMN sessions.audit_status IS 'Estado de auditoría: pending, synced, error';
COMMENT ON COLUMN sessions.mm IS 'Mes del conteo';
COMMENT ON COLUMN sessions.yyyy IS 'Año del conteo';

...
export const CameraScanner: React.FC<CameraScannerProps> = ({ onScan, onClose, inline = true, isTriggered = false }) => {
    ...
    const startNativeEngine = async () => {
        try {
            ...
            // BLINDAJE CRÍTICO: Prevenir "Illegal Constructor"
            let detector: any;
            try {
                // @ts-ignore
                ...
                console.warn("Native constructor blocked, fallback to WASM", instantiationError);
                startLegacyEngine();
                return;
            }
    ...
    const handleSuccessfulScan = (code: string) => {
        ...
        onScan(code);
        setTimeout(() => { if (isComponentMounted.current) setFeedbackStatus(null); }, 350);
    };
    ...
    return (
        <div className={`${inline ? 'w-full h-full relative' : 'fixed inset-0 z-[100]'} bg-black overflow-hidden`}>
            ...
            <div className={`absolute inset-0 z-40 transition-all duration-75 pointer-events-none flex flex-col items-center justify-center ${isTriggered ? 'bg-transparent opacity-0' : 'bg-black/80 backdrop-blur-sm'}`}>
                {!isTriggered && !error && <div className="flex flex-col items-center gap-4"><div className="bg-slate-900 border-4 border-white/5 p-5 rounded-full text-blue-500 shadow-2xl"><Lock className="w-10 h-10" /></div><span className="text-[10px] font-black text-white/30 uppercase tracking-[0.5em]">LENS_LOCKED</span></div>}
            </div>
            ...
            <div className="flex-1 relative bg-black flex flex-col justify-center h-full">
                {feedbackStatus === 'success' && <div className="absolute inset-0 z-[60] bg-emerald-600/60 backdrop-blur-md flex items-center justify-center animate-in zoom-in duration-100"><CheckCircle2 className="w-20 h-20 text-white" /></div>}
            ...
            <style>{`#${SCANNER_DOM_ID} video { width: 100% !important; height: 100% !important; object-fit: cover !important; }`}</style>
        </div>
    );
};

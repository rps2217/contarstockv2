...
export const ProductSearchInput: React.FC<Props> = ({
  ...
  return (
    <div className={`p-6 rounded-[2rem] border-4 border-black space-y-4 ${
      theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' || theme === 'gray' ? 'bg-blue-500/5' : 'bg-blue-50'
    }`}>
      <div className="flex items-center justify-between">
        <h3 className={`text-xs font-black uppercase tracking-widest ${theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' || theme === 'gray' ? 'text-blue-400' : 'text-blue-600'}`}>
          ...
          <div className="relative">
            ...
            <div className="absolute right-4 top-1/2 -translate-y-1/2">
              {isSearching ? (
                ...
                <Search className={`w-5 h-5 ${theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' || theme === 'gray' ? 'text-slate-600' : 'text-muted'}`} />
              )}
            </div>
          </div>
        </div>
      ...
      {product && (
        <div className={`p-3 rounded-xl border-2 flex items-center gap-3 ${
          theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' || theme === 'gray' ? 'bg-black/40 border-blue-500/30' : 'bg-white border-blue-200 shadow-sm'
        }`}>
          <div className="w-10 h-10 rounded-lg bg-blue-500 flex items-center justify-center shrink-0">
            <Package className="w-5 h-5 text-white" />
          </div>
          <div className="min-w-0">
            <p className={`text-[11px] font-black uppercase truncate ${theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' || theme === 'gray' ? 'text-white' : 'text-slate-900'}`}>
              {product.name}
            </p>

...
export const useSyncManager = () => {
  ...
  const addLog = useCallback(
    (msg: string | object, type: 'info' | 'error' | 'success' = 'info') => {
  ...
  const handlePushConfig = async () => {
    ...
    try {
      ...
      addLog(`✗ Error al respaldar configuración: ${(error as Error).message}`, 'error');
    } finally {
  ...
  const handlePullConfig = async () => {
    ...
    try {
      ...
      addLog(`✗ Error al restaurar configuración: ${(error as Error).message}`, 'error');
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const refreshGroups = useCallback(async () => {
    ...
    setUiGroups(
      groups.map((g: UploadGroup) => ({ ...g, uiStatus: 'idle' as const, progress: undefined }))
    );
  ...
  const handleDownloadOrders = async () => {
    ...
    try {
      ...
      addLog(`✗ Error al descargar órdenes: ${(error as Error).message}`, 'error');
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleSyncAll = async () => {
    ...
    for (let i = 0; i < uiGroups.length; i++) {
      ...
      setUiGroups(prev =>
        prev.map((g, idx) =>
          idx === i ? { ...g, uiStatus: 'uploading', progress: 'Iniciando...' } : g
        )
      );
  ...
  const handleVerifyIntegrity = async () => {
    ...
    try {
      ...
      const erpGroups = Array.from(
        new Set(allScans.map(s => String(s.logisticsLabel || 'SIN_ERP')))
      );
      ...
      for (const erp of erpGroups) {
        ...
        if (summaryResponse.success) {
          ...
          if (diff === 0) {
            ...
            addLog(
              `⚠️ Discrepancia en ${erp}: Local(${localTotal}) > Nube(${cloudTotal}). Faltan ${diff} unidades por subir.`,
              'error'
            );
          } else {
            addLog(
              `⚠️ Discrepancia en ${erp}: Local(${localTotal}) < Nube(${cloudTotal}). Hay ${Math.abs(diff)} unidades extra en la nube.`,
              'info'
            );
          }
        } else {
          addLog(
            `✗ Fallo al obtener resumen de nube para ${erp}: ${summaryResponse.error}`,
            'error'
          );
        }
      }
      ...
      addLog(`✗ Error crítico en verificación: ${(error as Error).message}`, 'error');
    } finally {
      setIsProcessing(false);
    }
  };

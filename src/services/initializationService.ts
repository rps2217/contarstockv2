 ...
 */
export type InitStepState =
  'idle' | 'version_check' | 'config' | 'database' | 'ready' | 'offline' | 'purging' | 'migrating';
...
const InitializationService = {
  ...
  run: async (onStep: (step: InitStep) => void): Promise<void> => {
    ...
    try {
      ...
      logger.info(
        'INIT',
        `Database ready: ${dbResult.sessionCount} sessions, firstLaunch: ${dbResult.isFirstLaunch}`
      );

...
export const CommandPalette: React.FC<CommandPaletteProps> = ({ isOpen, onClose }) => {
  ...
  const commands: CommandItem[] = useMemo(() => [
    // Navegación
    {
      ...
      shortcut: 'Ctrl+1',
      action: () => { window.dispatchEvent(new CustomEvent('navigate', { detail: 'dashboard' })); onClose(); },
      category: 'navigation',
    },
    {
      id: 'nav-capture',
      ...
      shortcut: 'Ctrl+2',
      action: () => { window.dispatchEvent(new CustomEvent('navigate', { detail: 'capture' })); onClose(); },
      category: 'navigation',
    },
    {
      id: 'nav-expiry',
      ...
      shortcut: 'Ctrl+3',
      action: () => { window.dispatchEvent(new CustomEvent('navigate', { detail: 'expiry' })); onClose(); },
      category: 'navigation',
    },
    {
      id: 'nav-inventory',
      ...
      shortcut: 'Ctrl+4',
      action: () => { window.dispatchEvent(new CustomEvent('navigate', { detail: 'inventory' })); onClose(); },
      category: 'navigation',
    },
    {
      id: 'nav-reports',
      ...
      shortcut: 'Ctrl+5',
      action: () => { window.dispatchEvent(new CustomEvent('navigate', { detail: 'reports' })); onClose(); },
      category: 'navigation',
    },
    {
      id: 'nav-settings',
      ...
      shortcut: 'Ctrl+,',
      action: () => { window.dispatchEvent(new CustomEvent('navigate', { detail: 'settings' })); onClose(); },
      category: 'navigation',
    },
    // Acciones
    {
      ...
      shortcut: 'Ctrl+N',
      action: () => { window.dispatchEvent(new CustomEvent('quick-add', { detail: 'product' })); onClose(); },
      category: 'create',
    },
    {
      ...
      icon: <CalendarClock className="w-4 h-4" />,
      action: () => { window.dispatchEvent(new CustomEvent('quick-add', { detail: 'expiry' })); onClose(); },
      category: 'create',
    },
    {
      id: 'action-refresh',
      ...
      shortcut: 'Ctrl+R',
      action: () => { window.dispatchEvent(new CustomEvent('keyboard-refresh')); onClose(); },
      category: 'action',
    },
    {
      id: 'action-shortcuts',
      ...
      shortcut: '?',
      action: () => { window.dispatchEvent(new CustomEvent('show-shortcuts')); onClose(); },
      category: 'action',
    },
  ], [onClose]);
          ...
          >
            <div className="bg-surface border border-subtle rounded-2xl shadow-2xl overflow-hidden">
              ...
              <div className="flex items-center gap-3 px-4 py-4 border-b border-subtle">
                ...
                />
                <kbd className="hidden sm:inline-flex items-center gap-1 px-2 py-1 text-xs text-muted bg-elevated rounded border border-subtle">
                        ...
                        return (
                          ...
                          >
                            ...
                            <ChevronRight className={cn(
                              ...
                              globalIdx === selectedIndex ? 'text-blue-400 opacity-100' : 'opacity-0'
                            )} />
                          </button>
                        );
                      })}

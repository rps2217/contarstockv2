...
-- La tabla ya existe con columnas en camelCase (createdat, erporder, etc.)
-- Agregar columnas que faltan en snake_case para compatibilidad
...
-- La tabla usa camelCase (productName, providerName, etc.)
-- Agregar columna snake_case para compatibilidad con syncRegistry
...
-- La tabla está vacía, asegurar que tenga la estructura correcta
...
SELECT table_name, 
       ...
       (SELECT COUNT(*) FROM information_schema.table_constraints WHERE table_name = c.table_name AND constraint_type = 'PRIMARY KEY') as tiene_pk

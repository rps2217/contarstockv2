 ...
 */
export async function parseFileContent(file: File): Promise<{ headers: string[]; rows: RawRow[] }> {
...
export function autoDetectMappings(headers: string[]): ColumnMappings {
  ...
  // SKU
  const skuIndex = lowerHeaders.findIndex(
    h =>
      h.includes('sku') ||
      h.includes('codigo') ||
      h.includes('código') ||
  ...
  // Barcode
  const barcodeIndex = lowerHeaders.findIndex(
    h =>
  ...
  // Quantity
  const qtyIndex = lowerHeaders.findIndex(
    h =>
      h.includes('cantidad') ||
      h.includes('quantity') ||
      h.includes('qty') ||
  ...
  // Description
  const descIndex = lowerHeaders.findIndex(
    h =>
  ...
  const unitIndex = lowerHeaders.findIndex(
    h => h.includes('unidad') || h.includes('unit') || h.includes('umed')
  );
  if (unitIndex >= 0) mappings.unit = headers[unitIndex];

  // Location
  const locIndex = lowerHeaders.findIndex(
    h =>

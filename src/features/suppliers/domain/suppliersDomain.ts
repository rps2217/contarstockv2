...
export const evaluateProviderStatus = (provider: Provider): ProviderStatus => {
  return provider.hasExchange ? ProviderStatus.WITH_EXCHANGE : ProviderStatus.WITHOUT_EXCHANGE;
};

/**
 * Verifica si un proveedor coincide con la búsqueda
 */
export const providerMatchesSearch = (provider: Provider, query: string): boolean => {
  if (!query.trim()) return true;

  const normalizedQuery = normalizeText(query);
  const searchTerms = normalizedQuery.split(/\s+/).filter(Boolean);

  const searchableFields = [provider.name, provider.rut, provider.businessName, provider.email].map(
    normalizeText
  );

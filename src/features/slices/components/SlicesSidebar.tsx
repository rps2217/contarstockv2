...
export const SlicesSidebar: React.FC<Props> = ({
  ...
  const iconBgActiveClass = isLight
    ? 'bg-blue-100 text-blue-600'
    : isHighContrast
      ? 'bg-yellow-900/30 text-yellow-400'
      : 'bg-blue-500/20 text-blue-400';
  const iconBgInactiveClass = isLight
    ? 'bg-slate-100 text-muted'
    : isHighContrast
      ? 'bg-yellow-900/20 text-yellow-600'
      : 'bg-surface/60 text-slate-500';

  const sliceNameClass = isLight
    ? 'text-slate-800'
    : isHighContrast
      ? 'text-yellow-300'
      : 'text-white';
  ...
  const helpBgClass = isLight
    ? 'bg-blue-50 border-blue-200'
    : isHighContrast
      ? 'bg-yellow-900/20 border-yellow-400/30'
      : 'bg-blue-500/5 border-blue-500/10';
  const helpTextClass = isLight
    ? 'text-slate-600'
    : isHighContrast
      ? 'text-yellow-500'
      : 'text-muted';
    ...
    <div className={`${containerClass} border rounded-3xl p-4 space-y-4`}>
          ...
          return (
            ...
            >
              <div className="flex items-start gap-2.5 min-w-0">
                <div
                  className={`p-1.5 rounded-lg mt-0.5 shrink-0 ${isActive ? iconBgActiveClass : iconBgInactiveClass}`}
                >
                  <Filter className="w-3.5 h-3.5" />
                </div>
                <div className="min-w-0">
                  <span
                    className={`text-xs font-black block truncate transition-colors ${sliceNameClass}`}
                  >
                    {slice.name}
                  </span>
                  <span
                    className={`text-[9px] font-mono uppercase tracking-wider ${subtitleClass}`}
                  >
      ...
      <div className={`p-3 ${helpBgClass} border rounded-2xl`}>
        <p className={`text-[10px] leading-normal ${helpTextClass}`}>
          <strong>¿Qué es un Slice?</strong> Es una &quot;segmentación de tabla&quot; como en
          AppSheet. En lugar de procesar millones de filas, diseñas vistas compactas para la línea
          operativa.
        </p>
      </div>
    </div>

...
export const ReportDetail: React.FC<{ sessionId: string; onBack: () => void }> = ({ sessionId, onBack }) => {
 ...
 return (
 <div className={`flex flex-col h-screen ${theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' ? 'bg-brand-background' : 'bg-slate-50'}`}>
 <header className={`border-b px-4 py-3 flex items-center justify-between sticky top-0 z-20 ${
   theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' ? 'bg-brand-surface border-white/5' : 'bg-white border-slate-200'
 }`}>
 <div className="flex items-center gap-3">
 <button onClick={onBack} className={`p-2 rounded-full transition-colors ${theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' ? 'hover:bg-white/5 text-muted' : 'hover:bg-slate-100 text-slate-600'}`}><ChevronLeft className="w-5 h-5" /></button>
 <div><h2 className={`font-black leading-none uppercase ${theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' ? 'text-white' : 'text-slate-900'}`}>{session?.erpOrder}</h2><span className="text-[10px] text-muted font-bold uppercase">{session?.logisticsLabel}</span></div>
 </div>
 ...
 <div className={`bg-indigo-50 border-2 border-indigo-200 rounded-[2rem] p-6 mb-6 animate-in slide-in-from-top duration-300 ${
   theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' ? 'bg-indigo-950/30 border-indigo-500/20' : ''
 }`}>
 <div className="flex items-center justify-between mb-4">
 <div className="flex items-center gap-2">
 <Zap className="w-5 h-5 text-indigo-600" />
 <h3 className={`font-black uppercase text-xs tracking-widest ${theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' ? 'text-indigo-300' : 'text-indigo-900'}`}>Resultados del Detective IA</h3>
 </div>
 ...
 <div key={idx} className={`p-4 rounded-2xl border flex items-center justify-between shadow-sm ${
   theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' ? 'bg-brand-surface border-white/5' : 'bg-white border-indigo-100'
 }`}>
 <div>
 <div className="flex items-center gap-2">
 <span className={`font-black uppercase ${theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' ? 'text-white' : 'text-slate-900'}`}>{match.expectedOrder.internalId}</span>
 ...
 className={`p-2 text-rose-600 rounded-lg transition-colors ${theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' ? 'hover:bg-white/5' : 'hover:bg-rose-50'}`}
 ...
 <h4 className={`font-black uppercase truncate text-sm ${theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' ? 'text-white' : 'text-slate-900'}`}>{item.productName}</h4>
 ...
 <button onClick={() => sessionService.deleteSessionItem(sessionId, item.barcode)} className={`p-2 transition-colors ${theme === 'dark' || theme === 'night' || theme === 'high-contrast' || theme === 'appsheet-dark' ? 'text-slate-500 hover:text-rose-500' : 'text-secondary hover:text-rose-600'}`}><Trash2 className="w-5 h-5" /></button>
 </div>
 </div>
 );
 })}
